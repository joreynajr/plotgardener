...
#' Plot a Manhattan-like plot
#' 
#' @usage plotManhattanGeneral(
#'     data,
#'     sigVal = 5e-08,
#'     chrom = NULL,
#'     chromstart = NULL,
...
#'
#' @export
plotManhattanGeneral <- function(data, sigVal = 5e-08, chrom = NULL,
                            chromstart = NULL, chromend = NULL,
                            assembly = "hg38", fill = "black", pch = 19,
                            cex = 0.25, leadSNP = NULL,
                            sigLine = FALSE, sigCol = NULL,
    ...
    manhattan_range <- function(bedData, object) {
        if (is.null(object$range)) {
            object$range <- c(0, ceiling(max(bedData[, "p"])))
        }
    ...
    if (nrow(bed_data) > 0 & manInternal$txdbChecks == TRUE) {
        ...
        if (nrow(leadSNP_row) > 0) {
            ...
            point <- pointsGrob(
                x = leadSNP_row$pos, y = leadSNP_row$p,
                pch = manInternal$leadSNP$pch,
                gp = manInternal$gp,
                default.units = "native"
            )
            snp <- textGrob(
                label = leadSNP_row$snp, x = leadSNP_row$pos,
                y = unit(leadSNP_row$p, "native") + unit(1.5, "mm"),
                just = "bottom",
                gp = gpar(
                    fontsize = manInternal$leadSNP$fontsize,
                    col = manInternal$leadSNP$fontcolor
